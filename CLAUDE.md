...
- **firebase-mcp** - Official Firebase integration (Firestore, Auth, Storage, Messaging, Remote Config, Data Connect, Crashlytics)
- **agentic-control-framework** - Framework for building autonomous agents
- **mcp-file-context-server** - File context and analysis
- **Ref** - Reference documentation and API tools
- **desktop-commander** - Desktop system operations and automation
- **exa** - Advanced web search and research
- **mem0-memory-mcp** - Persistent memory and knowledge management
- **toolbox** - Collection of utility tools and services
- **mcp-sequentialthinking-tools** - Sequential problem solving and reasoning
- **github** - GitHub repository operations and management
- **browser-tools-mcp** - Browser automation and web interaction
- **clear-thought-mcp-server** - Clear thinking and reasoning tools
- **supabase-mcp** - Supabase database operations and management
- **railway-mcp** - Railway deployment and hosting platform operations
...
### Overview
MCP is an open protocol that enables LLMs to access external tools and data sources through specialized servers. It allows Claude to connect to databases, APIs, file systems, and other resources.

### Configuration File: `.mcp.json`
Located in project root. This file defines project-scoped MCP servers that are shared with the team:
```json
{
  "mcpServers": {
    "server-name": {
...
1. **Stdio Server** - Communicates via standard input/output (default)
2. **SSE Server** - Uses Server-Sent Events for real-time communication
3. **HTTP Server** - REST API communication

### Server Scopes
- **Local**: Private to current project and user (default)
- **Project**: Shared with team via `.mcp.json` 
- **User**: Available across all projects for current user

### CLI Commands
```bash
# Add servers
claude mcp add <name> <command> [args...]                    # Add stdio server (local scope)
claude mcp add -s project <name> <command>                   # Add project-scoped server
claude mcp add -s user <name> <command>                      # Add user-scoped server
...
# Authentication
/mcp                                                        # Interactive menu for authentication

# Start Claude Code as MCP server
claude mcp serve                                            # Expose Claude's tools to other clients
```
...
Reference external resources using the `@` syntax:
```
...
```bash
claude mcp add postgres-db /usr/local/bin/postgres-mcp-server \
  --connection-string "postgresql://localhost:5432/teacher_dashboard"
```
...
```bash
claude mcp add filesystem /path/to/filesystem-mcp-server \
  -e ROOT_DIR=/home/user/projects
```

### Security Considerations
⚠️ **Important Security Notes:**
- Only use trusted MCP servers - they have full access to their configured capabilities
- Third-party servers can expose you to prompt injection risks
- Be especially careful with servers that communicate over the internet
- Review server permissions before installation
- Use OAuth 2.0 authentication when available for remote servers

### Troubleshooting
1. **Server not responding**: Check if the server executable exists and has execute permissions
2. **Authentication issues**: Use `/mcp` command to re-authenticate
3. **Resource not found**: Verify the resource path syntax matches server expectations
4. **Performance issues**: Consider server location and network latency for remote servers

### MCP Server Comparison & Usage Strategy

#### Context7 vs Ref MCP Servers

**Context7 MCP Server** 🎯 **Best for Discovery & Code Examples**
- **Library Discovery**: Finds 50+ related libraries with trust scores (0-10)
- **Rich Metadata**: Trust scores, code snippet counts, version info
- **Code-Focused**: Provides detailed code examples with context and explanations
- **Topic Filtering**: Can search for specific topics like "runes"
- **Comparative Analysis**: Shows multiple options with ratings to help choose

**Ref MCP Server** 🔍 **Best for Official Documentation & Web Search**
- **Official Documentation**: Direct access to official docs (e.g., svelte.dev)
- **Web Search**: Broader web search capabilities across the internet
- **URL Reading**: Can fetch and convert any webpage to markdown
- **Real-time Content**: Always up-to-date official documentation
- **Comprehensive Coverage**: Full official documentation depth

#### Recommended Usage Strategy

**Use Context7 when you want to:**
- Discover libraries and compare options
- Find code examples and implementations  
- Learn new concepts with practical examples
- Evaluate trust/quality of libraries
- Get comprehensive tutorials with code snippets

**Use Ref when you want to:**
- Read official documentation 
- Get the latest information from official sources
- Search the broader web for resources
- Convert web pages to readable markdown
- Access real-time content

**Best Practice: Use Both Together! 🚀**
1. **Start with Context7** to discover libraries and see code examples
2. **Use Ref** to get the official documentation for specific concepts
3. **Context7** for finding component libraries (UI kits, utilities)
4. **Ref** for official API references and migration guides

Both servers complement each other perfectly - Context7 for discovery and examples, Ref for official documentation and web search!````