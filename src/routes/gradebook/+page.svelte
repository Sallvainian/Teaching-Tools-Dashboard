<script lang="ts">
	...
	async function handleAddAssignment() {
		if (categoryId && assignmentName.trim()) {
			await gradebookStore.addAssignmentToCategory(
				assignmentName.trim(),
				maxPoints || 0,
				categoryId
			);
		...
		{:else}
				...
				<div class="flex-1">
					<div class="card-dark p-4">
						...
						<div class="flex gap-2">
							<select id="class-selector" class="select w-full" bind:value={categoryId}>
				...
				<div class="card-dark mb-6">
						...
						<div class="flex gap-2">
							...
							<button class="btn btn-sm btn-secondary">
								...
								>
									...
									<path d="M5 21h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v11a2 2 0 002 2z"
									></path>
								</svg>
								Import
							</button>
					...
					{#if hasData() && categoryAssignments.length > 0}
										...
										...categoryAssignments.map((assignment) => ({
											...
											validator: function (value: any, callback: any) {
												callback(
													value === null ||
														value === '' ||
														(value >= 0 && value <= assignment.maxPoints)
												);
											},
											allowInvalid: false,
											renderer: function (
						...
						<div class="mt-4 text-sm text-text-base">
							<p>
								<span class="font-medium">Pro Tip:</span> This gradebook supports Excel-like
								features:
					...
					{:else}
						<div class="flex flex-col items-center justify-center py-16 bg-surface/30 rounded-lg">
							...
							<p class="text-text-base text-center max-w-md mb-6">
								Add students to this class to start tracking grades. You can add students
								individually or import a list.
							</p>
										...
										<tr class="border-b border-border/50 hover:bg-surface/50 transition-colors">
											...
											<td class="py-3">
												{#if categoryStudents.length > 0}
													...
													{@const avg =
														grades.length > 0
															? ((total / grades.length / assignment.maxPoints) * 100).toFixed(1)
															: '—'}
								...
								<div class="bg-surface/50 rounded-lg p-4 hover:bg-surface transition-colors">
									<div class="flex items-center gap-3">
										...
										<div>
											...
											{#if categoryAssignments.length > 0}
												{@const avg = gradebookStore.studentAverageInCategory(
													student.id,
													categoryId
												)}
												<div class="text-sm">
													<span
														class={`${avg >= 90 ? 'text-green-400' : avg >= 80 ? 'text-blue-400' : avg >= 70 ? 'text-yellow-400' : avg >= 60 ? 'text-orange-400' : 'text-red-400'}`}
													>
														{avg}%
													</span>
			...
			{:else}
				<div class="card-dark p-8 text-center">
					...
					<p class="text-text-base mb-6">
						Please select a class from the dropdown above or create a new one to view grades.
					</p>
					<button onclick={toggleNewClassModal} class="btn btn-primary">
						<svg
							class="w-5 h-5 mr-2"
							viewBox="0 0 24 24"
							fill="none"
		...
		<div class="bg-card border border-border rounded-lg p-6 w-full max-w-md">
			...
			<div class="mb-6">
				...
				<input
					...
					class="input w-full"
				/>
				<p class="text-xs text-text-base mt-1">
					You'll be able to add students and assignments after creating the class.
				</p>
			</div>

			<div class="flex gap-3">
				<button type="button" onclick={toggleNewClassModal} class="btn btn-secondary flex-1">
					Cancel
				</button>
				<button
					type="button"
	...
	<div class="fixed inset-0 bg-bg-base/80 backdrop-blur-sm flex items-center justify-center z-50">
		<div class="bg-card border border-border rounded-lg p-6 w-full max-w-md">
			...
			<div class="mb-6">
				...
				>
				<input type="number" id="max-points" bind:value={maxPoints} min="1" class="input w-full" />
			</div>
