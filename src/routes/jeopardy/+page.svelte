      ...
      <div class="lg:col-span-2">
        {#if editingQuestion.categoryId}
          ...
          <div class="bg-dark-card border border-dark-border p-6 rounded-xl shadow-dark-card mb-6">
            ...
            <div class="space-y-4">
              <div>
                <label for="question-text" class="block text-sm text-dark-lavender font-medium mb-2"
                  >Question Text</label
                >
                <textarea
                  id="question-text"
              ...
              <div>
                <label for="answer-text" class="block text-sm text-dark-lavender font-medium mb-2"
                  >Answer</label
                >
                <input
                  id="answer-text"
              ...
              <div class="mt-2">
                ...
                {#if editingQuestion.isDoubleJeopardy}
                  <p class="text-xs text-dark-muted mt-1 ml-6">
                    Players can wager up to their current score or the question point value,
                    whichever is higher
                  </p>
                {/if}
              </div>
          ...
          {#if $getActiveGame.categories.length === 0}
            <div class="bg-dark-surface rounded-lg p-8 text-center">
              <p class="text-dark-muted">
                No categories added yet. Add your first category to start building your game.
              </p>
            </div>
          {:else}
            <div
            ...
            >
              ...
              {#each $getActiveGame.categories as category}
                <div
                  class="p-3 bg-dark-accent font-medium text-white text-center border-r border-dark-border"
                >
                  ...
                  {#if category.questions[rowIndex]}
                    ...
                    >
                      ...
                      {#if category.questions[rowIndex].isDoubleJeopardy}
                        <span
                          class="absolute top-1 right-1 w-3 h-3 bg-dark-highlight rounded-full"
  ...
  {:else}
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
        {#each $getActiveGame.teams as team}
          <div
            class="p-4 rounded-lg flex items-center justify-between transition-all"
            style="background-color: {team.color}; {team.id === $getLeadingTeam?.id
              ? 'box-shadow: 0 0 15px ' + team.color + ';'
              : ''}"
          >
            <span class="font-medium text-white">{team.name}</span>
      ...
      <div class="bg-dark-surface border-2 border-dark-border rounded-xl p-4 shadow-lg">
        {#if $getActiveGame.categories.length === 0}
          <div class="p-8 text-center">
            <p class="text-dark-muted">
              This game has no categories yet. Switch to Edit Mode to build your game.
            </p>
          </div>
        {:else}
          <div
              ...
              {#each $getActiveGame.categories as category}
                {#if category.questions[rowIndex]}
                  {#if category.questions[rowIndex].isAnswered}
                    ...
                    <div
                      class="p-6 sm:p-8 md:p-10 border border-dark-border bg-dark-card bg-opacity-50 flex items-center justify-center"
                    >
                  ...
                  {:else}
                    ...
                    >
                      ...
                      {#if category.questions[rowIndex].isDoubleJeopardy}
                        <span
                          class="absolute top-2 right-2 w-4 h-4 bg-dark-purple rounded-full"
                          title="Double Jeopardy"
                        ></span>
                      {/if}
                    </button>
                ...
                {:else}
                  <!-- Empty question cell -->
                  <div
                    class="p-6 sm:p-8 md:p-10 border border-dark-border bg-dark-card bg-opacity-60"
                  ></div>
                {/if}
              {/each}
            {/each}
